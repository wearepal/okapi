...
class RegionProposalNetworkWILDS(RegionProposalNetwork):
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        for objectness_, regression_targets_, labels_, objectness_, pred_bbox_deltas_ in zip(
            objectness, regression_targets, labels, objectness, pred_bbox_deltas
        ):
...
class FastWILDS(GeneralizedRCNN, MetaModel):
    def __init__(
        self,
        model: Model,
        ...
        box_batch_size_per_image: float = 512,
        ...
        image_mean = [0.0, 0.0, 0.0]  # small trick because images are already normalized
        image_std = [1.0, 1.0, 1.0]
        transform = GeneralizedRCNNTransform(min_size, max_size, image_mean, image_std)

        super().__init__(model, rpn, roi_heads, transform)
...
class FasterRCNNLoss(nn.Module):
    ...
    def forward(self, outputs: Tensor, targets: Tensor) -> Tensor:
        ...
        try:
            ...
            elementwise_loss = outputs.new_ones(size=(len(outputs),))
